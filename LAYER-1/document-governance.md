...
* не должен фигурировать в navigation как рабочий путь
...
1. Один тип authority = один canonical source в своей зоне.
2. Adapter files не содержат policy.
3. Runtime state допускается только в STATE.md.
4. DEPRECATED и ARCHIVED документы не участвуют в runtime.
5. Любой новый документ должен получить: role, authority, status, updated_by — до начала использования.
6. Если документ дублирует authority другого документа — он должен быть downgraded, deprecated или merged.
7. При downgrade или deprecation документа — все ссылки на него в navigation-файлах (`llms.txt`, `ARCHITECTURE.md` и др.) обновляются в той же операции.
