...
5. При любом изменении файлов в `LAYER-1/` или `LAYER-2/` (навигационно значимых) — обновить `LAYER-1/navigation.md` в том же коммите.

---

## Правила сопровождения документации

### Обязательная синхронизация `LAYER-1/navigation.md`

`LAYER-1/navigation.md` — это обязательная карта и индекс документации в `LAYER-1/` и `LAYER-2/`.

При любом изменении документации агент обязан поддерживать `LAYER-1/navigation.md` в актуальном состоянии в рамках того же изменения.

Обязательные действия:
- Если создан новый навигационно значимый файл в `LAYER-1/` или `LAYER-2/` — добавить соответствующую строку в `LAYER-1/navigation.md`.
- Если файл документации переименован, перемещён или изменён его путь — обновить соответствующую строку в `LAYER-1/navigation.md`.
- Если файл документации удалён — удалить соответствующую строку из `LAYER-1/navigation.md`.
- Если у документа изменились смысл, статус, область применения или заголовок — обновить описание и связанные поля в `LAYER-1/navigation.md`.
- Нельзя оставлять такие изменения без синхронного обновления `LAYER-1/navigation.md` в том же коммите, патче или pull request.

Операционное правило:
- Любое изменение дерева документации считается незавершённым, пока не обновлён `LAYER-1/navigation.md`.
- Перед завершением задачи нужно проверить, что все актуальные документы отражены в `LAYER-1/navigation.md`.
- Нужно удалять устаревшие строки, которые ссылаются на несуществующие файлы.
- Нельзя откладывать обновление `LAYER-1/navigation.md` на потом.
...
- Исключение допускается только если в задаче явно указано не изменять `LAYER-1/navigation.md`.

Проверка перед завершением:
- Все актуальные файлы документации перечислены в `LAYER-1/navigation.md`.
- В таблице нет строк для удалённых или перемещённых файлов.
- Пути, названия и описания актуальны.
- Обновление `LAYER-1/navigation.md` входит в то же изменение, что и правка документации.
...
**Docs Agent** — обновляет CHANGELOG, HANDOFF, LAYER-3/, LAYER-1/navigation.md
