...
def usage():
    print("Usage: python3 scripts/agent-fail.py --dry-run --task-id TASK_ID [--queue-dir PATH]")
    print("Usage: python3 scripts/agent-fail.py --approve-fail --task-id TASK_ID [--queue-dir PATH]")
    return 2
...
def parse_args(argv):
    ...
    while index < len(argv):
        ...
        if arg == "--dry-run":
            ...
            return None
    if dry_run == approve_fail or not task_id:
        return None
    return {
        "dry_run": dry_run,
...
def main():
    ...
    print("Execution or verification failed.")
    print("Do you approve marking this task as failed/blocked?")
    print("Do not move task.")
    print("Do not mark failed.")
    return 0
