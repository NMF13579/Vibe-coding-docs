...
Use `ROUTES-REGISTRY.md` only to confirm canonical module ownership.
Do not browse files independently.
Do not invent structure or rules.
If the task is clear, do it inside the confirmed scope.
If something is missing or contradictory, briefly say what is missing and stop.

When validating adapters, use `scripts/ADAPTER-SPEC.md`.
