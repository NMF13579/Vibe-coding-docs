...
**Если `HANDOFF.md` не существует** → это новый проект. Переключиться на протокол `START.md` и предложить выбор уровня разработки.

**Если `HANDOFF.md` существует** (обычная сессия):
1. Прочитай `HANDOFF.md` и `memory-bank/project-status.md`.
2. Прочитай `memory-bank/lessons-learned.md` — чтобы не повторять прошлые ошибки.
...
- `session-guard-agent` → `.claude/agents/session-guard-agent.md` — следит за сохранением контекста.
